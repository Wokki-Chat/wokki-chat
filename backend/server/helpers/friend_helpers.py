...
async def send_friend_request(sid, data):
    ...
    if access_token is None or friend_username is None:
        addMessageToLogs(f"Missing required fields for send_friend_request", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            ...
            if not user_id:
                addMessageToLogs(f"Invalid access token for send_friend_request, access token: {access_token}", "INFO")
            ...
            if not friend_row:
                addMessageToLogs(f"User not found for send_friend_request, username: {friend_username}", "INFO")
                await sio_instance.sio.emit('friend_request_sent', {'success': False, 'msg': 'User not found'}, to=sid)
                return
            
            if user_id == friend_row['id']:
                addMessageToLogs(f"Cannot send friend request to yourself for send_friend_request, user id: {user_id}", "INFO")
            ...
            if await is_user_friends_with(cur, user_id, friend_row['id']):
                addMessageToLogs(f"Already friends for send_friend_request, user id: {user_id}, friend id: {friend_row['id']}", "INFO")
            ...
            addMessageToLogs(f"Friend request sent for send_friend_request, user id: {user_id}, friend id: {friend_row['id']}", "INFO")
            ...
            if friend_sid:
                addMessageToLogs(f"Friend request received for send_friend_request, user id: {user_id}, friend id: {friend_row['id']}", "INFO")
...
async def accept_friend_request(sid, data):
    ...
    if access_token is None or requested_friend_id is None:
        addMessageToLogs(f"Invalid data for accept_friend_request", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            user_id = await verify_access_token(cur, access_token)
            if not user_id:
                addMessageToLogs(f"Invalid access token for accept_friend_request, access token: {access_token}", "INFO")
            ...
            friend_row = await cur.fetchone()
            if not friend_row:
                addMessageToLogs(f"User not found for accept_friend_request, user id: {requested_friend_id}", "INFO")
                await sio_instance.sio.emit('accept_friend_request_response', {'success': False, 'msg': 'User not found'}, to=sid)
                return
            
            if await is_user_friends_with(cur, user_id, friend_row['id']):
                addMessageToLogs(f"Already friends for accept_friend_request, user id: {user_id}, friend id: {friend_row['id']}", "INFO")
            ...
            addMessageToLogs(f"Friend request accepted for accept_friend_request, user id: {user_id}, friend id: {friend_row['id']}", "INFO")
...
async def pending_friend_requests(sid, data):
    ...
    if access_token is None:
        addMessageToLogs(f"Invalid data for pending_friend_requests", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            user_id = await verify_access_token(cur, access_token)
            if not user_id:
                addMessageToLogs(f"Invalid access token for pending_friend_requests, access token: {access_token}", "INFO")
            ...
            addMessageToLogs(f"Pending friend requests for pending_friend_requests, user id: {user_id}", "INFO")
...
async def outgoing_friend_requests(sid, data):
    ...
    if access_token is None:
        addMessageToLogs(f"Invalid data for outgoing_friend_requests", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            user_id = await verify_access_token(cur, access_token)
            if not user_id:
                addMessageToLogs(f"Invalid access token for outgoing_friend_requests, access token: {access_token}", "INFO")
            ...
            addMessageToLogs(f"Outgoing friend requests for outgoing_friend_requests, user id: {user_id}", "INFO")
...
async def cancel_outgoing_friend_request(sid, data):
    ...
    if access_token is None or friend_id is None:
        addMessageToLogs(f"Invalid data for cancel_outgoing_friend_request", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            user_id = await verify_access_token(cur, access_token)
            if not user_id:
                addMessageToLogs(f"Invalid access token for cancel_outgoing_friend_request, access token: {access_token}, friend id: {friend_id}", "INFO")
            ...
            if await is_user_friends_with(cur, user_id, friend_id):
                addMessageToLogs(f"Already friends for cancel_outgoing_friend_request, user id: {user_id}, friend id: {friend_id}", "INFO")
            ...
            await conn.commit()
            addMessageToLogs(f"Deleted outgoing friend request for cancel_outgoing_friend_request from database, user id: {user_id}, friend id: {friend_id}", "INFO")
            ...
            addMessageToLogs(f"Outgoing friend request canceled for cancel_outgoing_friend_request, user id: {user_id}, friend id: {friend_id}", "INFO")
...
async def deny_friend_request(sid, data):
    ...
    if access_token is None or friend_id is None:
        addMessageToLogs(f"Invalid data for deny_friend_request", "INFO")
        return
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            user_id = await verify_access_token(cur, access_token)
            if not user_id:
                addMessageToLogs(f"Invalid access token for deny_friend_request, access token: {access_token}, friend id: {friend_id}", "INFO")
                await sio_instance.sio.emit('deny_friend_request_response', {'success': False, 'msg': 'Invalid access token'}, to=sid)
                return
            
            if await is_user_friends_with(cur, user_id, friend_id):
                addMessageToLogs(f"Already friends for deny_friend_request, user id: {user_id}, friend id: {friend_id}", "INFO")
            ...
            await conn.commit()
            addMessageToLogs(f"Deleted incoming friend request for deny_friend_request from database, user id: {user_id}, friend id: {friend_id}", "INFO")
            await sio_instance.sio.emit('deny_friend_request_response', {'success': True, 'msg': 'Friend request denied'}, to=sid)
            addMessageToLogs(f"Friend request denied for deny_friend_request, user id: {user_id}, friend id: {friend_id}", "INFO")