...
async def typing(sid, metadata, data):
    ...
    async with typing_lock:
        ...
        await sio_instance.sio.emit('users_typing', {'user_ids': list(typing_users), 'channel_id': channel_id, 'server_id': server_id}, room=f'server:{server_id}:channel:{channel_id}')
