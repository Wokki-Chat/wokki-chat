...
async def typing(sid, data):
    ...
    async with config.pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            ...
            if not user_id:
                await addMessageToLogs(f"Invalid access token for typing, access token: {access_token}", "INFO")
                await sio_instance.sio.emit('error', {'msg': 'Invalid access token'}, to=sid)
                return
            
            if not await is_user_in_server(cur, user_id, server_id):
                await addMessageToLogs(f"User is not in server for typing, user id: {user_id}, server id: {server_id}", "INFO")
